var DecibelMeter = ( function ( window, navigator, document, undefined ) {
  ...
  if (navigator.mediaDevices) { // && navigator.mediaDevices.getUserMedia
    navigator.getUserMedia = function(constraints, onStream, onError) {
    ...
    navigator.getUserMedia = window.OTPlugin.getUserMedia.bind(window.OTPlugin);
  }
	...
	navigator.mediaDevices.enumerateDevices().then(function(srcs){
		srcs.forEach( function (source) {
			if (source.kind === 'audiooutput' || source.kind === 'audioinput') {
				sources.push(source);
